...
class Brain:
    def __init__(self, cfg):
        ...
        if (self.exploration_rate_decay is None) and (self.exploration_rate_min != 0.) and (self.exploration_rate != 0):
    ...
    def select_action(self, state, eval=False):
        ...
        if np.random.rand() < self.exploration_rate and not eval:
            ...
            with torch.no_grad():
                v = self.policy_net(after_states).squeeze(-1)
    ...
    def _loss(self, batch, weights):
        ...
        with torch.no_grad():
            next_v = self.policy_net(next_state)
